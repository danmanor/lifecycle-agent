...
func newFakeIPClient(t *testing.T, objs ...*ipcv1.IPConfig) *fake.ClientBuilder {
	...
	if len(objs) > 0 {
		objsAny := make([]crclient.Object, 0, len(objs))
		for _, o := range objs {
			objsAny = append(objsAny, o)
		}
		builder = builder.WithObjects(objsAny...).WithStatusSubresource(objsAny...)
	}
	return builder
}
...
func Test_writeIPConfigRunConfig_WriteError(t *testing.T) {
	...
	ipc := &ipcv1.IPConfig{
		Spec: ipcv1.IPConfigSpec{
			IPv4: &ipcv1.IPFamilyConfig{Address: "192.0.2.10/24"},
		},
	}
...
func Test_RunLcaCliIPConfigRun_SuccessAndFailure(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to schedule lca-cli ip-config run")
}
...
func Test_runLcaCliIPConfigPrepare_SuccessAndFailure(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to schedule lca-cli ip-config prepare")
}
...
// --- validateStageTransition / validateDNSMasqMCExists ---

func Test_validateStageTransition(t *testing.T) {
	// invalid stage
	client := newFakeIPClient(t).Build()
	...
	err := handler.validateStageTransition(context.Background(), ipc)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "invalid IPConfig stage")

	// valid stage but missing dnsmasq MC
	ipc.Spec.Stage = ipcv1.IPStages.Config
	ipc.Status.ValidNextStages = []ipcv1.IPConfigStage{ipcv1.IPStages.Config}
	err = handler.validateStageTransition(context.Background(), ipc)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "validation of DNSMasq machine config failed")

	// success when MC exists
	...
	sch := scheme.Scheme
	_ = machineconfigv1.AddToScheme(sch)
	successClient := fake.NewClientBuilder().WithScheme(sch).WithObjects(mc).Build()
	handler.Client = successClient
	err = handler.validateStageTransition(context.Background(), ipc)
	assert.NoError(t, err)
}
...
func Test_IPConfigConfigPhasesHandler_PreConfiguration_Flow(t *testing.T) {
	...
	mockOps.EXPECT().RunSystemdAction(gomock.Any()).Return("ok", nil)
...
func Test_IPConfigConfigPhasesHandler_PreConfiguration_ErrorPaths(t *testing.T) {
	...
	mockOps.EXPECT().RunSystemdAction(gomock.Any()).Return("", fmt.Errorf("systemd error"))
...
func Test_IPConfigConfigStageHandler_Handle_PrepareUnknown_PrePivotCalled(t *testing.T) {
	...
	status := common.IPConfigRunStatus{Phase: common.IPConfigPhaseUnknown}
...
func Test_IPConfigConfigStageHandler_Handle_PrepareRunning(t *testing.T) {
	...
	status := common.IPConfigRunStatus{Phase: common.IPConfigPhaseRunning}
	data, _ := json.Marshal(status)
...
func Test_IPConfigConfigStageHandler_Handle_PrepareFailed(t *testing.T) {
	...
	status := common.IPConfigRunStatus{Phase: common.IPConfigPhaseFailed, Message: "prepare failed"}
	data, _ := json.Marshal(status)
...
func Test_IPConfigConfigStageHandler_Handle_PrepareSucceeded_TargetNotBooted(t *testing.T) {
	...
	status := common.IPConfigRunStatus{Phase: common.IPConfigPhaseSucceeded}
	data, _ := json.Marshal(status)
...
func Test_IPConfigConfigStageHandler_Handle_PrepareSucceeded_RunStatusReadError(t *testing.T) {
	...
	prepStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseSucceeded}
...
func Test_IPConfigConfigStageHandler_Handle_PrepareSucceeded_RunRunning(t *testing.T) {
	...
	prepStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseSucceeded}
	prepData, _ := json.Marshal(prepStatus)
	runStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseRunning}
...
func Test_IPConfigConfigStageHandler_Handle_PrepareSucceeded_RunUnknown_DelegatesPreConfiguration(t *testing.T) {
	...
	prepStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseSucceeded}
	prepData, _ := json.Marshal(prepStatus)
	runStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseUnknown}
	runData, _ := json.Marshal(runStatus)
...
func Test_IPConfigConfigStageHandler_Handle_PrepareSucceeded_RunFailed(t *testing.T) {
	...
	prepStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseSucceeded}
	prepData, _ := json.Marshal(prepStatus)
	runStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseFailed, Message: "run failed"}
	runData, _ := json.Marshal(runStatus)
...
func Test_IPConfigConfigStageHandler_Handle_PrepareSucceeded_RunSucceeded_DelegatesPostConfiguration(t *testing.T) {
	...
	prepStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseSucceeded}
	prepData, _ := json.Marshal(prepStatus)
	runStatus := common.IPConfigRunStatus{Phase: common.IPConfigPhaseSucceeded}
	runData, _ := json.Marshal(runStatus)
